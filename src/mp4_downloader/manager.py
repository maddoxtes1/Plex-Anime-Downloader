from src.mp4_downloader.scrap import *
from src.mp4_downloader.downloaders import *
from src.mp4_downloader.segmentation import *
...
class manager:
    def __init__(self, download_path, url):
    ...
    def sibnet(self):
        ...
        if mp4_url:
            ...
            status = mp4_downloader(headers, mp4_url, self.file_name)
            return status
        else:
            return False
    
    def vidmoly(self):
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36',"Referer": "https://vidmoly.to/"}
        segmentation_path = f"{self.temp_path}/segmentation/"
        segments_txt = f"{self.temp_path}/segments.txt"
        if not os.path.exists(segmentation_path):
            os.makedirs(segmentation_path)
        segments = vidmoly_scrap(self.url, segmentation_path)

        downloader = Segment_Downloader(segments=segments, headers=headers, segments_txt=segments_txt)
        downloader.download_segments_in_parallel()
        status = merge_segments(self.file_name, self.temp_path)
        return status
    
    def oneupload(self):
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36',"Referer": "https://oneupload.net/"}
        segmentation_path = f"{self.temp_path}/segmentation/"
        segments_txt = f"{self.temp_path}/segments.txt"
        if not os.path.exists(segmentation_path):
            os.makedirs(segmentation_path)
    ...
    def sendvid(self):
        mp4_url = sendvid_scrap(self.url)
        if mp4_url:
